...
from app.schemas.user import UserInDB, CreateCheckoutSession, SubscriptionResponse
...
async def handle_subscription_updated(subscription, db: AsyncSession):
    ...
    if status == "active":
        user.subscription_tier = SubscriptionTier.PRO if subscription.get("items", {}).get("data", [{}])[0].get("price", {}).get("unit_amount", 0) > 2000 else SubscriptionTier.BASIC
